...
export async function POST(request: NextRequest) {
  try {
    ...
    const project = db.prepare('SELECT * FROM AgencyProject WHERE id = ?').get(projectId) as { name: string; deadline: string | null } | undefined
    db.close()

    // Карточка в канбане ПМ для каждой подзадачи
      ...
      try {
        await fetch(`${pmBoardUrl}/api/cards`, {
          ...
          body: JSON.stringify({
            ...
            source_detail_id: id,
            name: `${project.name} — ${String(title)}`,
            deadline: project.deadline || null,
            status: 'not_started',
          }),
        })
      } catch (e) {
        console.error('PM board create card for detail failed:', e)
      }
    }
