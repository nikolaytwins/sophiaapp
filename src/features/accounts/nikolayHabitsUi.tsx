    ...
    >
      ...
      {hasNumbers ? (
        <>
          ...
          >
            {fmtRub(current)} ₽
            <Text style={{ fontSize: 15, fontWeight: '600', color: colors.textMuted }}> / {fmtRub(target)} ₽</Text>
          </Text>
          <View
            style={{
              marginTop: 14,
              height: 12,
              borderRadius: 999,
              backgroundColor: isLight ? 'rgba(15,17,24,0.08)' : 'rgba(255,255,255,0.08)',
              overflow: 'hidden',
            }}
          >
            <View
              style={{
                ...
                height: '100%',
                borderRadius: 999,
                backgroundColor: 'rgba(201,168,108,0.85)',
              }}
            />
          </View>
          <Text style={{ marginTop: 8, fontSize: 12, fontWeight: '600', color: colors.textMuted }}>
            {Math.round(pct * 100)}% · активный спринт
          </Text>
        </>
      ) : (
        <Text style={{ marginTop: 14, fontSize: 14, lineHeight: 21, color: colors.textMuted }}>{fallbackHint}</Text>
      )}
    </AppSurfaceCard>
  );
}
...
function SprintLinkRow() {
  const { spacing, brand } = useAppTheme();
  return (
...
/** Крупные плашки Китай и подушка на экране «День». */
...
export function NikolayDayFocusPanel({
  ...
  return (
    <>
      <NikolayDayMoneyHeroCards chinaGoal={chinaGoal} cushionGoal={cushionGoal} />
